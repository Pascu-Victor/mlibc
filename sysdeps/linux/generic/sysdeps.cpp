...
void Sysdeps<LibcPanic>::operator()() { __builtin_trap(); }
...
#if defined(__riscv)
int Sysdeps<RiscvHwprobe>::operator()(
...
static auto vdso_clock_gettime =
    reinterpret_cast<int (*)(clockid_t, struct timespec *)>(VDSO_SYMBOL("__vdso_clock_gettime"));
...
Sysdeps<Sigaction>::operator()(int signum, const struct sigaction *act, struct sigaction *oldact) {
	...
	auto ret = do_syscall(
	    SYS_rt_sigaction,
	    signum,
	    act ? &kernel_act : nullptr,
	    oldact ? &kernel_oldact : nullptr,
...
void Sysdeps<Yield>::operator()() { do_syscall(SYS_sched_yield); }
...
int Sysdeps<Renameat>::operator()(
    int old_dirfd, const char *old_path, int new_dirfd, const char *new_path
) {
#ifdef SYS_renameat2
...
void Sysdeps<Sync>::operator()() { do_syscall(SYS_sync); }
	...
	switch (evp ? evp->sigev_notify : SIGEV_SIGNAL) {
		...
		case SIGEV_THREAD_ID: {
			if (evp) {
				...
				ksev.sigev_tid =
				    (evp && evp->sigev_notify == SIGEV_THREAD_ID) ? evp->sigev_notify_thread_id : 0;
				ksevp = &ksev;
			}
...
int Sysdeps<NameToHandleAt>::operator()(
...
int Sysdeps<Msgrcv>::operator()(
    int msqid, void *msgp, size_t msgsz, long msgtyp, int msgflg, ssize_t *out
) {
...
int
Sysdeps<MqOpen>::operator()(const char *name, int oflag, mode_t mode, mq_attr *attr, mqd_t *out) {
...
int Sysdeps<Setxattr>::operator()(
    const char *path, const char *name, const void *val, size_t size, int flags
) {
...
int
Sysdeps<Fsetxattr>::operator()(int fd, const char *name, const void *val, size_t size, int flags) {
	auto ret = do_syscall(SYS_fsetxattr, fd, name, val, size, flags);
	return sc_error(ret);
}

int Sysdeps<Getxattr>::operator()(
    const char *path, const char *name, void *val, size_t size, ssize_t *nread
) {
...
int
Sysdeps<Fgetxattr>::operator()(int fd, const char *name, void *val, size_t size, ssize_t *nread) {
...
int Sysdeps<Statx>::operator()(
    int dirfd, const char *path, int flags, unsigned int mask, struct statx *statxbuf
) {
...
int Sysdeps<ProcessVmReadv>::operator()(
    pid_t pid,
	...
	auto ret =
	    do_syscall(SYS_process_vm_readv, pid, local_iov, liovcnt, remote_iov, riovcnt, flags);
	if (int e = sc_error(ret); e)
		return e;
	*out = sc_int_result<ssize_t>(ret);
	return 0;
...
int Sysdeps<ProcessVmWritev>::operator()(
    pid_t pid,
    const struct iovec *local_iov,
    unsigned long liovcnt,
    const struct iovec *remote_iov,
	...
	auto ret =
	    do_syscall(SYS_process_vm_writev, pid, local_iov, liovcnt, remote_iov, riovcnt, flags);
	if (int e = sc_error(ret); e)
		return e;
	*out = sc_int_result<ssize_t>(ret);
	return 0;
...
int
Sysdeps<Fsmount>::operator()(int fsfd, unsigned int flags, unsigned int mountflags, int *outfd) {
	if (!fsfd)
		return EINVAL;
...
int
Sysdeps<Fsconfig>::operator()(int fd, unsigned int cmd, const char *key, const void *val, int aux) {
...
int Sysdeps<MoveMount>::operator()(
    int from_dirfd, const char *from_path, int to_dirfd, const char *to_path, unsigned int flags
) {
...
int Sysdeps<CopyFileRange>::operator()(
    int fd_in,
    off_t *off_in,
    int fd_out,
    off_t *off_out,
