...
size_t mbrtowc(
    wchar_t *__restrict wcp, const char *__restrict mbs, size_t mb_limit, mbstate_t *__restrict stp
) {
	auto cc = mlibc::current_charcode();
...
size_t mbsrtowcs(
    ...
    const char **__restrict mbsp,
    size_t wc_limit,
    mbstate_t *__restrict stp
) {
	__ensure(mbsp);
	...
	if (!wcs) {
		size_t size;
		if (auto e = cc->decode_wtranscode_length(nseq, &size, *stp);
...
size_t wcsrtombs(
	...
	if (!mbs) {
		size_t size;
		if (auto e = cc->encode_wtranscode_length(wseq, &size, *stp);
		    e != mlibc::transcode_status::null_terminator) {
			__ensure(
			    e == mlibc::transcode_status::illegal_input
			    || e == mlibc::transcode_status::input_underflow
