...
struct utf8_charcode {
	...
	struct decode_state {
		...
		decode_state(int progress, codepoint cpoint) : _progress{progress}, _cpoint{cpoint} {}
...
struct polymorphic_charcode_adapter : polymorphic_charcode {
	...
	// Nulls are not written into wseq; instead, `transcode_status::null_terminator` is returned.
	transcode_status decode_wtranscode(
	...
	transcode_status encode_wtranscode(
	...
	// returned.
	transcode_status encode_wtranscode_length(
