...
struct category_item {
	...
	category_item(uint32_t v) { value = v; }

	category_item(frg::span<const uint8_t> v) { value = v; }

	category_item(frg::span<const uint32_t> v) {
		value = frg::span{reinterpret_cast<const uint8_t *>(v.data()), v.size() * sizeof(uint32_t)};
	}

	category_item(frg::string_view v) { value = v; }

	category_item(frg::basic_string_view<wchar_t> v) { value = v; }
	...
	uint32_t asUint32() const { return std::get<uint32_t>(value); }
	...
	frg::span<const uint16_t> asUint16Span() const {
		...
		return frg::span{
		    reinterpret_cast<const uint16_t *>(span.data()), span.size() / sizeof(uint16_t)
		};
	}
	...
	frg::span<const uint32_t> asUint32Span() const {
		...
		auto span = std::get<frg::span<const uint8_t>>(value);
		return frg::span{
		    reinterpret_cast<const uint32_t *>(span.data()), span.size() / sizeof(uint32_t)
		};
	}
...
struct nl_ctype : nl_category<LC_CTYPE, _NL_NUM_LC_CTYPE> {
	...
	frg::span<const uint32_t> class_table(size_t index) {
		...
		size_t entrySize =
		    (index + 1 < offsets.size()) ? offsets[index + 1] - offsets[index] : distanceToEnd;
		entrySize &= ~3;

		return frg::span(
		    reinterpret_cast<const uint32_t *>(data.data() + offsets[index]), entrySize
		);
	}

	frg::span<const uint16_t> ctype_class() { return get(_NL_CTYPE_CLASS).asUint16Span(); }

	frg::span<const uint32_t> map_tolower() { return get(_NL_CTYPE_TOLOWER).asUint32Span(); }

	frg::span<const uint32_t> map_toupper() { return get(_NL_CTYPE_TOUPPER).asUint32Span(); }

	frg::span<const uint8_t> ctype_class32() { return get(_NL_CTYPE_CLASS32).asByteSpan(); }

	frg::string_view class_names() { return get(_NL_CTYPE_CLASS_NAMES).asString(); }

	frg::string_view map_names() { return get(_NL_CTYPE_MAP_NAMES).asString(); }

	uint32_t class_offset() { return get(_NL_CTYPE_CLASS_OFFSET).asUint32(); }

	uint32_t map_offset() { return get(_NL_CTYPE_MAP_OFFSET).asUint32(); }
