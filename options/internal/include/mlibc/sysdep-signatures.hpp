...
using SysdepOf = std::conditional_t<std::derived_from<ST, Tag>, mlibc::SysdepImpl<Tag>, NoImpl>;

#define SYSDEP_FUNC(name, ...)                                                                     \
	template <>                                                                                    \
	struct SysdepImpl<name> {                                                                      \
		static int operator()(__VA_ARGS__);                                                        \
	}
#define SYSDEP_FUNC_RET(rettype, name, ...)                                                        \
	template <>                                                                                    \
	struct SysdepImpl<name> {                                                                      \
		static rettype operator()(__VA_ARGS__);                                                    \
	}
#define SYSDEP_FUNC_NORETURN(name, ...)                                                            \
	template <>                                                                                    \
	struct SysdepImpl<name> {                                                                      \
		[[noreturn]] static void operator()(__VA_ARGS__);                                          \
	}
...
SYSDEP_FUNC(
    Stat, mlibc::fsfd_target fsfdt, int fd, const char *path, int flags, struct stat *statbuf
);
SYSDEP_FUNC(
    VmMap, void *hint, size_t size, int prot, int flags, int fd, off_t offset, void **window
);
...
SYSDEP_FUNC(
    GetSockopt, int fd, int layer, int number, void *__restrict buffer, socklen_t *__restrict size
);
...
SYSDEP_FUNC(
    Accept, int fd, int *newfd, struct sockaddr *addr_ptr, socklen_t *addr_length, int flags
);
...
SYSDEP_FUNC(
    Sockname, int fd, struct sockaddr *addr_ptr, socklen_t max_addr_length, socklen_t *actual_length
);
SYSDEP_FUNC(
    Peername, int fd, struct sockaddr *addr_ptr, socklen_t max_addr_length, socklen_t *actual_length
);
...
SYSDEP_FUNC(
    TimerfdSettime, int fd, int flags, const struct itimerspec *value, struct itimerspec *oldvalue
);
...
SYSDEP_FUNC(
    Openpty, int *mfd, int *sfd, char *name, const struct termios *ios, const struct winsize *win
);
