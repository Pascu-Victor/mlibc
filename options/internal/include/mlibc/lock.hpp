...
struct alignas(4) FutexLockImpl {
		...
		while (true) {
			if (!expected) {
				...
				if (__atomic_compare_exchange_n(
				        &_state, &expected, this_tid, false, __ATOMIC_ACQUIRE, __ATOMIC_ACQUIRE
				    )) {
				...
				if ((expected & ownerMask) == this_tid) {
					if constexpr (Recursive)
						...
						mlibc::panicLogger()
						    << "mlibc: FutexLock deadlock detected!" << frg::endlog;
					return;
				}
	...
	void unlock() {
		...
		if (state & waitersBit) {
			// Wake the futex if there were waiters. Since the mutex might not exist at this
			// location anymore, we must conservatively ignore EACCES and EINVAL which may occur as
			// a result.
