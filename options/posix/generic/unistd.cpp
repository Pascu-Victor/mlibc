...
char *getcwd(char *buffer, size_t size) {
	...
	if (int e = mlibc::sysdep_or_panic<Stat>(
	...
	for (;;) {
		...
		if (int e = mlibc::sysdep_or_panic<Stat>(
		        mlibc::fsfd_target::fd, par_dir, nullptr, 0, &par_dir_stat
		    );
		    e) {
...
int lchown(const char *path, uid_t uid, gid_t gid) {
	if (int e = mlibc::sysdep_or_enosys<Fchownat>(AT_FDCWD, path, uid, gid, AT_SYMLINK_NOFOLLOW);
	    e) {
		errno = e;
		return -1;
	}
