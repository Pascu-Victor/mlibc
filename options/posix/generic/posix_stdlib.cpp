...
// Borrowed from musl
static uint32_t init[] = {0x00000000, 0x5851f42d, 0xc0b18ccf, 0xcbb5f646, 0xc7033129, 0x30705b04,
...
double drand48(void) { return erand48(seed_48); }
...
long jrand48(unsigned short s[3]) { return (int32_t)(eand48_step(s, seed_48 + 3) >> 16); }

long int mrand48(void) { return jrand48(seed_48); }
...
int mkostemp(char *pattern, int flags) { return mkostemps(pattern, 0, flags); }

int mkstemp(char *path) { return mkostemp(path, 0); }

int mkstemps(char *pattern, int suffixlen) { return mkostemps(pattern, suffixlen, 0); }
	...
	auto process_segment = [&](frg::string_view s_view) -> int {
		if (mlibc::globalConfig().debugPathResolution)
			mlibc::infoLogger() << "mlibc realpath(): resolv is '" << resolv.data() << "'"
			                    << ", segment is " << s_view.data() << ", size: " << s_view.size()
			                    << frg::endlog;
		...
		if (mlibc::globalConfig().debugPathResolution)
			mlibc::infoLogger() << "mlibc realpath(): stat()ing '" << resolv.data() << "'"
			                    << frg::endlog;
		struct stat st;
		if (int e = mlibc::sysdep_or_panic<Stat>(
		        mlibc::fsfd_target::path, -1, resolv.data(), AT_SYMLINK_NOFOLLOW, &st
		    );
		    e)
			return e;
		...
		if (S_ISLNK(st.st_mode)) {
			if (mlibc::globalConfig().debugPathResolution) {
				mlibc::infoLogger() << "mlibc realpath(): Encountered symlink '" << resolv.data()
				                    << "'" << frg::endlog;
			}
			...
			if (mlibc::globalConfig().debugPathResolution) {
				mlibc::infoLogger()
				    << "mlibc realpath(): Symlink resolves to '"
			...
			if (path[0] == '/') {
				...
				if (mlibc::globalConfig().debugPathResolution) {
					mlibc::infoLogger()
					    << "mlibc realpath(): Symlink is relative, resolv: '" << resolv.data()
					    << "' lnk: '" << frg::string_view{lnk.data(), lnk.size()} << "'"
					    << frg::endlog;
				}
			}
		}
	...
	while (ps < path_view.size()) {
		...
		while (ls < lnk.size()) {
			...
			if (auto slash = strchr(lnk.data() + ls, '/'); slash) {
				ls_view = frg::string_view{
				    lnk.data() + ls, static_cast<size_t>(slash - (lnk.data() + ls))
				};
			} else {
...
size_t strxfrm_l(char *__restrict dest, const char *__restrict src, size_t n, locale_t) {
	// TODO: strxfrm_l should take "LC_COLLATE" into account.
	size_t l = strlen(src);
	if (n > l)
		strncpy(dest, src, n);
	return l;
...
int getsubopt(
    char **__restrict__ optionp, char *const *__restrict__ keylistp, char **__restrict__ valuep
) {
	*valuep = nullptr;
	...
	if (*s) {
		// If there's an equals sign, set the value pointer, and skip over the value part of the
		// token. Terminate the token.
...
char *canonicalize_file_name(const char *name) { return realpath(name, nullptr); }
