	...
	: _file(file),
	  ...
	  _function(function) {
		if (!mlibc::globalConfig().debugPthreadTrace)
			return;
		mlibc::infoLogger() << "trace: Enter scope " << _file << ":" << _line << " (in function "
...
static_assert(
    sizeof(__mlibc_sched_param) == sizeof(struct sched_param),
    "__mlibc_sched_param needs to be identical to struct sched_param"
);

int pthread_attr_getschedparam(
    const pthread_attr_t *__restrict attr, struct sched_param *__restrict schedparam
) {
	memcpy(schedparam, &attr->__mlibc_schedparam, sizeof(*schedparam));
	return 0;
}
int pthread_attr_setschedparam(
    pthread_attr_t *__restrict attr, const struct sched_param *__restrict schedparam
) {
...
int pthread_attr_getaffinity_np(
    const pthread_attr_t *__restrict attr, size_t cpusetsize, cpu_set_t *__restrict cpusetp
) {
	if (!attr)
		return EINVAL;
	...
	auto p = memcpy(cpusetp, attr->__mlibc_cpuset, std::min(cpusetsize, attr->__mlibc_cpusetsize));
...
int pthread_attr_setaffinity_np(
    pthread_attr_t *__restrict attr, size_t cpusetsize, const cpu_set_t *__restrict cpusetp
) {
	if (!attr)
		return EINVAL;
...
int
pthread_attr_getsigmask_np(const pthread_attr_t *__restrict attr, sigset_t *__restrict sigmask) {
	if (!attr)
		return EINVAL;
...
int
pthread_attr_setsigmask_np(pthread_attr_t *__restrict attr, const sigset_t *__restrict sigmask) {
	if (!attr)
		return EINVAL;
...
int get_own_stackinfo(void **stack_addr, size_t *stack_size) {
	...
	if (!fp) {
		mlibc::infoLogger() << "mlibc pthreads: /proc/self/maps does not exist! Returning ENOSYS."
		                    << frg::endlog;
		return ENOSYS;
	}
...
int pthread_setaffinity_np(pthread_t thread, size_t cpusetsize, const cpu_set_t *mask) {
	return mlibc::sysdep_or_enosys<SetThreadaffinity>(
	    reinterpret_cast<Tcb *>(thread)->tid, cpusetsize, mask
	);
}
...
int pthread_create(
    ...
    const pthread_attr_t *__restrict attrp,
    void *(*entry)(void *),
...
pthread_t pthread_self(void) { return reinterpret_cast<pthread_t>(mlibc::get_current_tcb()); }
...
__attribute__((__noreturn__)) void pthread_exit(void *ret_val) {
	...
	while (1) {
		...
		if (__atomic_compare_exchange_n(
		        &self->cancelBits, &old_value, new_value, true, __ATOMIC_RELAXED, __ATOMIC_RELAXED
		    ))
			break;
	}
...
int pthread_join(pthread_t thread, void **ret) { return mlibc::thread_join(thread, ret); }

int pthread_detach(pthread_t thread) { return mlibc::thread_detach(thread); }
...
void sigcancel_handler(int signal, siginfo_t *info, void *ucontext) {
	...
	if (signal != SIGCANCEL || info->si_pid != getpid() || info->si_code != SI_TKILL)
		return;
	 ...
	 */
	if (!(old_value & tcbCancelAsyncBit) && mlibc::IsImplemented<BeforeCancellableSyscall>
	    && !mlibc::sysdep_or_enosys<BeforeCancellableSyscall>(uctx))
		return;
...
int pthread_setcanceltype(int type, int *oldtype) {
	...
	while (1) {
		...
		if (oldtype)
			*oldtype =
			    ((old_value & tcbCancelAsyncBit) ? PTHREAD_CANCEL_ASYNCHRONOUS
...
void pthread_testcancel(void) { mlibc::thread_testcancel(); }
...
int
pthread_mutexattr_getrobust(const pthread_mutexattr_t *__restrict attr, int *__restrict robust) {
...
int
pthread_condattr_getclock(const pthread_condattr_t *__restrict attr, clockid_t *__restrict clock) {
	*clock = attr->__mlibc_clock;
	return 0;
}

int pthread_condattr_setclock(pthread_condattr_t *attr, clockid_t clock) {
	if (clock != CLOCK_REALTIME && clock != CLOCK_MONOTONIC && clock != CLOCK_MONOTONIC_RAW
	    && clock != CLOCK_REALTIME_COARSE && clock != CLOCK_MONOTONIC_COARSE
...
namespace {
int rwlock_m_lock(
    pthread_rwlock_t *rw, bool excl, const struct timespec *__restrict timeout = nullptr
) {
	...
	while (true) {
		if (m_expected) {
			...
			int e = mlibc::sysdep<FutexWait>(
			    (int *)&rw->__mlibc_m, m_expected | mutex_waiters_bit, timeout
			);
			if (e != 0 && e != EAGAIN && e != EINTR)
				return e;
			...
			if (__atomic_compare_exchange_n(
			        &rw->__mlibc_m, &m_expected, desired, false, __ATOMIC_ACQUIRE, __ATOMIC_RELAXED
			    ))
				break;
		}
	}
...
int
pthread_rwlock_init(pthread_rwlock_t *__restrict rw, const pthread_rwlockattr_t *__restrict attr) {
	SCOPE_TRACE();
	rw->__mlibc_m = 0;
	rw->__mlibc_rc = 0;
...
int pthread_rwlock_clockwrlock(
    pthread_rwlock_t *rw, clockid_t clock, const struct timespec *__restrict abstime
) {
	SCOPE_TRACE();
	...
	while (true) {
		...
		// Wait on the futex.
		int e = mlibc::sysdep<FutexWait>(
		    (int *)&rw->__mlibc_rc, rc_expected | rc_waiters_bit, &timeout
		);
		if (e != 0 && e != EAGAIN && e != EINTR) {
			rwlock_m_unlock(rw);
			return e;
...
int pthread_getcpuclockid(pthread_t, clockid_t *) {
	mlibc::infoLogger() << "mlibc: pthread_getcpuclockid() always returns ENOENT" << frg::endlog;
	return ENOENT;
}
...
int pthread_spin_lock(pthread_spinlock_t *__lock) {
	...
	while (true) {
		if (__atomic_compare_exchange_n(
		        &__lock->__lock, &expected, desired, true, __ATOMIC_ACQUIRE, __ATOMIC_RELAXED
		    ))
			break;
		if (expected == desired)
			return EDEADLK;
...
int pthread_spin_trylock(pthread_spinlock_t *__lock) {
	...
	if (!__atomic_compare_exchange_n(
	        &__lock->__lock, &expected, desired, false, __ATOMIC_ACQUIRE, __ATOMIC_RELAXED
	    ))
		return EBUSY;
