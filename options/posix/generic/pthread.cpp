...
constinit frg::array<key_global_info, PTHREAD_KEYS_MAX> key_globals_{};
...
int pthread_detach(pthread_t thread) {
	...
	if (!__atomic_compare_exchange_n(
	        &tcb->isJoinable, &expected, 0, false, __ATOMIC_RELEASE, __ATOMIC_RELAXED
	    ))
		return EINVAL;
...
void sigcancel_handler(int signal, siginfo_t *info, void *ucontext) {
	 ...
	 */
	if (!(old_value & tcbCancelAsyncBit) && mlibc::sys_before_cancellable_syscall
	    && !mlibc::sys_before_cancellable_syscall(uctx))
		return;
...
int pthread_once(pthread_once_t *once, void (*function)(void)) {
	...
	while (!(expected & onceComplete)) {
		if (!expected) {
			...
			if (int e = mlibc::sys_futex_wait((int *)&once->__mlibc_done, onceLocked, nullptr);
			    e && e != EINTR && e != EAGAIN)
...
int pthread_barrier_wait(pthread_barrier_t *barrier) {
	if (barrier->__mlibc_flags != 0) {
		mlibc::panicLogger() << "mlibc: pthread_barrier_t flags were non-zero" << frg::endlog;
	}
...
int pthread_rwlock_trywrlock(pthread_rwlock_t *rw) {
	...
	if (rw->__mlibc_flags != 0) {
		mlibc::panicLogger() << "mlibc: pthread_rwlock_t flags were non-zero" << frg::endlog;
	}
