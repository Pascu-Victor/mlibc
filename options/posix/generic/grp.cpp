...
bool extract_entry(frg::string_view line, group *entry) {
	...
	size_t n_members = 0;
	walk_segments(segments[3], ',', [&](frg::string_view) { n_members++; });
...
void endgrent(void) { close_global_file(); }
...
struct group *getgrgid(gid_t gid) {
	...
	int err = walk_file(&entry, [&](group *entry) { return entry->gr_gid == gid; });
...
int getgrgid_r(gid_t gid, struct group *grp, char *buffer, size_t size, struct group **result) {
	*result = nullptr;
	int err = walk_file(grp, [&](group *entry) { return entry->gr_gid == gid; });
...
struct group *getgrnam(const char *name) {
	...
	int err = walk_file(&entry, [&](group *entry) { return !strcmp(entry->gr_name, name); });
...
int
getgrnam_r(const char *name, struct group *grp, char *buffer, size_t size, struct group **result) {
	*result = nullptr;

	int err = walk_file(grp, [&](group *entry) { return !strcmp(entry->gr_name, name); });
...
int putgrent(const struct group *g, FILE *f) {
	auto invalid = [](const char *s) { return s == nullptr || strchr(s, '\n') || strchr(s, ':'); };
