...
int fstatat(int dirfd, const char *path, struct stat *result, int flags) {
	if (int e =
	        mlibc::sysdep_or_enosys<Stat>(mlibc::fsfd_target::fd_path, dirfd, path, flags, result);
	    e) {
		errno = e;
		return -1;
	}
...
int lstat(const char *path, struct stat *result) {
	if (int e = mlibc::sysdep_or_enosys<Stat>(
	        mlibc::fsfd_target::path, -1, path, AT_SYMLINK_NOFOLLOW, result
	    );
	    e) {
		errno = e;
		return -1;
