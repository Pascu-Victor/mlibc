...
#elif defined(__riscv)
	auto hwcap = getauxval(AT_HWCAP);
	return reinterpret_cast<ifunc_handler>(addr)(hwcap, &__riscv_hwprobe, nullptr);
#elif defined(__loongarch64)
	ifunc_arg ifunc_arg = {
	    ._size = sizeof(ifunc_arg),
...
frg::expected<LinkerError, SharedObject *> ObjectRepository::requestObjectWithName(
	...
	frg::expected<LinkerError, frg::unique_ptr<SharedObject, MemoryAllocator>> res =
	    LinkerError::notFound;

	auto trySharedObjectSetup =
	    [&](frg::string<MemoryAllocator> path,
	        int fd) -> frg::expected<LinkerError, frg::unique_ptr<SharedObject, MemoryAllocator>> {
		auto object = frg::make_unique<SharedObject>(
		    getAllocator(), name.data(), std::move(path), false, localScope, rts
		);
...
size_t ObjectSymbol::size() { return _symbol->st_size; }
...
frg::optional<ObjectSymbol> resolveInObject(
	...
	if (object->hashStyle == HashStyle::systemV) {
		...
		auto hash_table = reinterpret_cast<const GnuHashTableHeader *>(
		    object->baseAddress + object->hashTableOffset
		);
		auto buckets = reinterpret_cast<const uint32_t *>(
		    object->baseAddress + object->hashTableOffset + sizeof(GnuHashTableHeader)
